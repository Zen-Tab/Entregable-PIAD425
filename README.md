## Repositorio para los Trabajos Finales de Ingenieria de Software con IA

## Respuesta a las preguntas:
**1. ¿Qué producto genera mayores ventas?**
El SSD 256GB registra las ventas más altas con S/. 1,200.

**2. ¿Qué ciudad tiene mayor volumen de ventas?**
Lima concentra el mayor volumen total: S/. 2,655.

**3. ¿Cuál es el promedio de ventas?**
El promedio por producto es S/. 696.5.

**4. ¿Existe mucha variación en las ventas?**
Existe variación significativa, con desviación estándar de 278.64 (rango: S/. 375 a 1,200).

**5. ¿Qué producto debería promocionarse más según los datos?**
El Pendrive 32GB (S/. 375), que presenta las ventas más bajas.

## Autor
Jhonatan Najarro Mejia.